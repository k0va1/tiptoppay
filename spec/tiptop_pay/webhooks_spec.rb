...
describe TiptopPay::Webhooks do
  describe "HMAC validation" do
    ...
    context "with data_valid?" do
      it "returns true on valid hmac" do
        expect(TiptopPay.webhooks.data_valid?(data, valid_hmac)).to be_truthy
      end

      it "returns false on invalid hmac" do
        expect(TiptopPay.webhooks.data_valid?(data, invalid_hmac)).to be_falsey
      end
    end

    context "with validate_data!" do
      it "returns true on valid hmac" do
        expect(TiptopPay.webhooks.validate_data!(data, valid_hmac)).to be_truthy
      end

      it "raises a HMACError on invalid hmac" do
        expect {
          TiptopPay.webhooks.validate_data!(data, invalid_hmac)
        }.to raise_error(TiptopPay::Webhooks::HMACError)
      end
    end
  end
  ...
  describe "on_check" do
    ...
    subject { TiptopPay.webhooks.on_check(raw_data) }
  end
  ...
  describe "on_pay" do
    ...
    subject { TiptopPay.webhooks.on_pay(raw_data) }
  ...
  describe "on_fail" do
    ...
    subject { TiptopPay.webhooks.on_fail(raw_data) }
  ...
  describe "on_recurrent" do
    ...
    subject { TiptopPay.webhooks.on_recurrent(raw_data) }
